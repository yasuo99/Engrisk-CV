...
namespace Application.Services.Core
{
    public class PostService : IPostService
    {
        ...
        {
            var posts = await _context.Posts.Where(post => post.VerifiedStatus == status).Include(inc => inc.Account).Include(inc => inc.PostImages).Include(inc => inc.Comments).OrderByDescending(orderBy => orderBy.CreatedDate).ToListAsync();
            if(search != null){
