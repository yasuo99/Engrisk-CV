...
namespace Application.Services.Core
{
    public class QuestionService : IQuestionService
    {
        ...
        {
            var questions = await _context.Questions.Include(inc => inc.Answers).OrderByDescending(orderBy => orderBy.CreatedDate).AsNoTracking().ToListAsync();
            if (search != null)
            {
            ...
            {
                questions = questions.Where(question => question.Status == status).ToList();
            }
        ...
        {
            var questions = await _context.Questions.Where(q => q.Status == QuestionStatus.Free).Include(inc => inc.Answers).AsNoTracking().ToListAsync();
            if (type != QuestionType.None)
            {
                questions = questions.Where(question => question.Type == type).ToList();
            }
